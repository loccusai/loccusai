...
const LandingPage = ({ onStart, theme, toggleTheme }: { onStart: () => void; theme: string; toggleTheme: () => void; }) => {
    ...
    <div className="landing-page">
        ...
        <section className="hero">
            ...
            <button className="hero-cta" onClick={onStart}>Começar Teste Gratuito</button>
        ...
        <section className="cta-section"><h2 className="section-title">Pronto para dominar o mercado local para seus clientes?</h2><button className="hero-cta" onClick={onStart}>Comece seu teste gratuito</button></section>
...
const AnalysisToolPage = ({ onBack, onAnalysisComplete, theme, toggleTheme, onNavigateToProposalBuilder }: { onBack: () => void; onAnalysisComplete: (result: AnalysisResult, companyName: string) => void; theme: string, toggleTheme: () => void; onNavigateToProposalBuilder: (analysis: AnalysisHistoryItem) => void; }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const { responseText: rawResponse, groundingChunks } = await analyzeCompanyPresence(companyName, city, state, keywordsArray, userLocation);
    ...
    const renderContent = () => {
        if (!session) {
            switch (currentView) {
                ...
                default: return <LandingPage onStart={() => setCurrentView('login')} theme={theme} toggleTheme={toggleTheme} />;
            }
        }
